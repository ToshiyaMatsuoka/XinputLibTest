...
public:
	...
	*/
	void LoadTexture(LPCWSTR FilePath, std::string textureKey);
	/**
	* @brief 画像表示
	* @param textureKey 画像の格納キー
	...
	* @param color 色
	*/
	void DrawWord(RECT rect, LPCWSTR text, std::string FontNumber, int TextFormat = DT_LEFT, DWORD color = 0xff000000);
	/**
	* @brief DXフォント文字設定
	* @param height 文字の高さ
	* @param width 文字の幅
	...
	*/
	void SetFont(int height, int width, std::string FontKey, LPCWSTR FontType = L"ＭＳ　ゴシック", int CharSet = DEFAULT_CHARSET);
	/**
	* @brief DXフォント消去
	* @param FontKey 表示文字設定の格納先配列番号
	*/
	...
	*/
	HRESULT BuildDXDevice(HWND hWnd, bool isWindowMode, LPCWSTR FilePath);
	/**
	...
	* @param FilePath デバイス生成チェック用画像ファイルパス
	*/
	HRESULT RecoverDevice(HWND hWnd, bool isWindowMode, LPCWSTR FilePath);
...
private:
	...
	HRESULT InitD3d(HWND hWnd, LPCWSTR FilePath);
