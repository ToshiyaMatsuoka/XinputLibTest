...
//ダイレクト3Dの初期化関数
HRESULT DirectX::InitD3d(HWND hWnd, LPCWSTR FilePath)
{
	HRESULT hr = NULL;
	// 「Direct3D」オブジェクトの作成
	...
	if (FAILED(hr = m_pDirect3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd,
		...
		&d3dpp, &m_pD3Device)))
	{
		MessageBox(0, L"HALモードでDIRECT3Dデバイスを作成できません\nREFモードで再試行します", NULL, MB_OK);
...
HRESULT DirectX::BuildDXDevice(HWND hWnd, bool WinMode, LPCWSTR FilePath) {
	//ダイレクト３Dの初期化関数を呼ぶ
	...
	{
		MessageBox(0, L"DirectInputDeviceの作成に失敗しました", L"", MB_OK);
		return E_FAIL;
	}
...
void DirectX::LoadTexture(LPCWSTR FilePath, string textureKey) {
...
void DirectX::DrawWord(RECT rect, LPCWSTR text, string FontNumber, int TextFormat, DWORD color) {
...
void DirectX::SetFont(int height, int width, string FontKey, LPCWSTR FontType, int CharSet) {
	D3DXCreateFont(
		m_pD3Device,
		height,
		width,
...
HRESULT DirectX::RecoverDevice(HWND hWnd, bool WinMode, LPCWSTR FilePath) {
